#!/bin/Rscript
##' Time-stamp: <liuminzhao 07/27/2013 23:36:04>
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  mod1 <- ToursMNAR(y, R, X, tau = 0.05, method = 'heter2', niter = 2000, init = init1)
  mod3 <- ToursMNAR(y, R, X, tau = 0.3, method = 'heter2', niter = 2000)
  mod5 <- ToursMNAR(y, R, X, tau = 0.5, method = 'heter2', niter = 2000)
  mod7 <- ToursMNAR(y, R, X, tau = 0.7, method = 'heter2', niter = 2000)
  mod9 <- ToursMNAR(y, R, X, tau = 0.95, method = 'heter2', niter = 2000, init = init9)
