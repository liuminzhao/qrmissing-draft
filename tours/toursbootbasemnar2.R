#!/bin/Rscript
##' Time-stamp: <liuminzhao 07/14/2013 20:11:01>
...
rm(list=ls())
source('~/Documents/qrmissing/code/ToursMNAR.R')
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  mod1 <- ToursMNAR(y, R, X, tau = 0.05, method = 'heter2', niter = 2000)
  ...
  coefw2 <- c(coef1[1,], coef9[1,])
  coefw3 <- c(coef1[2,], coef9[2,])

  ans <- c(coefw2, coefw3, count1, count9)

}

write.table(result, file = "toursbootagebbasemnar-19.txt", row.names = FALSE, col.names = FALSE)
sendEmail(subject="boot tours base mnar 1-9", text="done", address="liuminzhao@gmail.com")
