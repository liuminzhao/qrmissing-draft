...
distributions and specify a sequential model parametrization for each multivariate normal distribution;  we specify the model in this way such that we have 
...
These distributions have a constraint such that the marginal
...
number of observed responses.   For identifiability of the observed data distribution, we use the
following restrictions,
\begin{displaymath}
 \sum_{k=1}^J \beta_{l1}^{(k)} = 0, l = 1,\ldots, p.
\end{displaymath}
$\Delta_{ij}$ subject/time specific intercepts determined by the parameters in (\ref{eq:model}); more details are given below.  
