...
% \author{Minzhao Liu\emailx{liuminzhao@ufl.edu} \\
%   Department of Statistics, University of Florida, FL 32601, USA
% \and
%   Michael J. Daniels\emailx{mjdaniels@austin.utexas.edu}\\
%   Section of Integrative Biology, Division of Statistics and Scientific
%   Computation, \\
...
\begin{keywords}
Monotone missingness; Nonignorable missingness; Quantile regression; Sensitivity analysis.
\end{keywords}
...
coefficients unconditional on random effects.  In addition, they are
...
proposed quantile regression model in section \ref{ch3:sec:settings}. We
provide details on MAR and MNAR and computation in sections
\ref{ch3:sec:sa} and \ref{ch3:sec:computation}.
...
\begin{equation}\label{ch3:eq:decompose}
...
\begin{displaymath}
  \prob (Y_{ij} \leq \bmath x_{ij}^T \bmath \gamma_j ) = \tau,
\end{displaymath}
where $\bmath \gamma_j$ is the $\tau^{th}$ quantile regression
...
section \ref{ch3:sec:sa}.  $\bmath x_{ij}$ is a $p \times 1$ covariate vector;
...
model. The mixture model in (\ref{ch3:eq:model}) allows the marginal
...
\end{displaymath}
Further details on these restrictions can be
found in Appendix \ref{ch3:sec:iden}.
...
(\ref{ch3:eq:decompose}). In this section, we explore ways to embed the
missingness mechanism and sensitivity parameters in mixture models for
our setting.

In the mixture model in (\ref{ch3:eq:model}), MAR holds \citep{molen1998,
...
$\Delta_{ij}$ in (\ref{ch3:eq:model}) for $j = 1, \ldots, J$. Then we show
how to obtain maximum likelihood estimates in section
\ref{ch3:sec:mle}. Finally, we present a Monte Carlo Markov Chain (MCMC)
sampling algorithm for Bayesian inference in section
\ref{ch3:sec:bayesian}.

\subsubsection{Calculation of $\Delta$ }
\label{ch3:sec:deltacal}
From equation (\ref{ch3:eq:deltaeqn1}) and (\ref{ch3:eq:deltaeqn2}),
...
\begin{itemize}
\item \textbf{$\Delta_{i1}: $} Expand equation (\ref{ch3:eq:deltaeqn1}):
  \begin{align*}
...
\item \textbf{$\Delta_{ij}, 2\leq j \leq J: $}
  ...
  \end{lem}
  Proof of \ref{ch3:sec:lemma} is in Appendix \ref{ch3:sec:proof}.

  Given the result in Lemma \ref{ch3:sec:lemma}, to solve equation
  (\ref{ch3:eq:deltaeqn2}), we propose a recursive approach. For the first
  multiple integral in equation (\ref{ch3:eq:deltaeqn2}), apply lemma
  ...
  each multiple integral in equation (\ref{ch3:eq:deltaeqn2}) can be
...
$S_i = k$ is
\begin{align} \label{ch3:eq:ll} L_i(\bmath \xi| \bmath y_i, S_{i} = k) & =
...
As mentioned in section \ref{ch3:sec:sa}, MAR or MNAR assumptions are
...
\item $\bmath \gamma_{1} $: Use Metropolis-Hasting algorithm.
  ...
  \item Draw ($\bmath \gamma_{1}^c$) candidates from candidate
    distribution;
  \item Based on the new candidate parameter $\bmath \xi^c$, calculate
    candidate $\Delta_{ij}^c$ for each subject $i$ as we described in
    section \ref{ch3:sec:deltacal}.
  \item Plug in $\Delta_{ij}^c$ in likelihood (\ref{ch3:eq:ll}) to get
...
section \ref{ch3:sec:settings} with the \textit{rq} function (noted as RQ) in
...
assumptions similar to \textit{rq} and assumes ignorable missing data.
...
fixed at $\bmath 0$ as discussed in section \ref{ch3:sec:sa}. For
...
Table \ref{ch3:tab:simh2}, \ref{ch3:tab:sim2} and \ref{ch3:tab:sim3} present the
...
residuals in model (\ref{ch3:eq:model}) to check the normality assumption
on the error term for a random sample of the simulated datasets
(Appendix \ref{ch3:sec:gofsim}).  When our error assumption is correct
...
when errors are not normal.
  ...
  \begin{sidewaystable}[ht]
    ...
    \caption{Scenario 1: MSE(MCSE) for coefficients estimates of
      ...
      \gamma_{12})$ are ones for $Y_{i2}$. MM stands for our proposed
      method, and RQ stands for the 'rq' function in R package
      'quantreg'. BZ stands for Bottai's approach.}\label{ch3:tab:simh2}
...
\end{sidewaystable}
  ...
  \begin{sidewaystable}[h]
    ...
    \caption{Scenario 2: MSE(MCSE) for coefficients estimates of
      ...
      \gamma_{12})$ are ones for $Y_{i2}$. MM stands for our proposed
      method, and RQ stands for the 'rq' function in R package
      'quantreg'. BZ stands for Bottai's approach.
      } \label{ch3:tab:sim2} \vspace{10pt} \tabcolsep =
    0.11cm
    \begin{tabular}{rrrrrrrrrrrrrrrr}
      \Hline
Normal      &  \multicolumn{3}{c}{0.1} &  \multicolumn{3}{c}{0.3} &  \multicolumn{3}{c}{0.5} &
...
$T_3$    &  \multicolumn{3}{c}{0.1} &  \multicolumn{3}{c}{0.3} &  \multicolumn{3}{c}{0.5} &
    ...
    \Hline
Laplace    & \multicolumn{15}{c}{MNAR Laplace} \\
    \cline{2-16}
...
\end{sidewaystable}
  ...
  \begin{sidewaystable}[h]
    ...
    \caption{Scenario 3: MSE(MCSE) for coefficients estimates of
      ...
      ones for $Y_{i2}$. MM stands for our proposed method, and RQ
      stands for the 'rq' function in R package 'quantreg'. BZ stands
      for Bottai's approach.} \label{ch3:tab:sim3}
    \vspace{10pt}
 \tabcolsep = 0.11cm
    \begin{tabular}{rrrrrrrrrrrrrrrr}
      \Hline
...
\ref{ch3:tab:tours}. For weights of participants at six months, weights of
...
\begin{table}[ht]
  ...
  \begin{center}
    \caption{Estimated marginal quantile regression coefficients with
      ...
      assumption is MAR.}\label{ch3:tab:tours}
    \vspace{10pt} \tabcolsep = 0.11cm
    \begin{tabular}{rrrrr}
      \Hline
...
Table \ref{ch3:tab:tours} presents the estimates and bootstrap percentile
...
\ref{ch3:sec:goftours}. The QQ plots showed minimal evidence against the
...
\citet{bottai2013} if error is mis-specified. We also illustrated
...
\begin{align}
  Y | R = 1 & \sim N(\Delta + \mu_1, \sigma_1), \label{ch3:eq:app1} \\
...
\begin{align*}
  \theta = \Delta + \mu_1\phi + \mu_0(1-\phi), \\
  \Delta = \theta - \phi \mu_1 - (1 - \phi)\mu_0.
\end{align*}
We can see $\Delta$ is determined by $\theta, \mu_1, \mu_0$. Plugging
in (\ref{ch3:eq:app1}), we have
\begin{align*}
  Y| R = 1 & \sim N(\theta + (1 - \phi)\mu_1 - (1 - \phi)\mu_0, \sigma_1), \\
  Y| R = 0 & \sim N(\theta - \phi \mu_1 + \phi \mu_0, \sigma_0).
\end{align*}
...
\end{align*}
Thus $\bmath \xi = (\theta, \mu_1)$ is identifiable. If $\xi_2 \neq
\xi_1$, then $\pr_2(y, R) \neq \pr_1(y, R)$.
...
$\bmath \xi = (\theta, \mu_1, \mu_0) $ is not identifiable.

Again by iterated expectations, we have
\begin{align*}
  \tau = \phi \Phi \left( \frac{\theta - \Delta - \mu_1}{\sigma_1}
  \right) + (1 - \phi) \Phi \left( \frac{\theta - \Delta -
      \mu_0}{\sigma_0} \right).
\end{align*}
Thus $\Delta$ is again determined by the other parameters:
\begin{align*}
  \Delta = h(\theta, \mu_1, \mu_0, \sigma_1, \sigma_0, \phi, \tau).
\end{align*}
To show $\bmath \xi = (\theta, \mu_1, \mu_0, \sigma_1, \sigma_0)$ is not
identifiable, we need to find $\bmath \xi^{'} \neq \bmath \xi$, such that
...
\begin{align*}
  h(\bmath \xi) + \mu_1 & = h(\bmath \xi^{'}) + \mu_1^{'},\\
  h(\bmath \xi) + \mu_0 & = h(\bmath \xi^{'}) + \mu_0^{'}.
\end{align*}
...
\begin{align*}
  ...
  = h(\bmath \xi) - \delta = \Delta - \delta.
\end{align*}
Then the new parameter $\bmath \xi^{'}$ yields the same distribution as
$\bmath \xi$. Therefore $\bmath \xi$ is not identifiable.

If we use a constraint, for example $\mu_1 = -\mu_0$, then
$\pr(y|R;\bmath \xi) = \pr(y|R; \bmath \xi^{'})$ yields $\bmath \xi = \bmath
\xi^{'}$.
...
\begin{align*}
  ...
  \sigma_1, \sigma_0, \phi, \tau).
\end{align*}
We want to show the parameter $\bmath \xi = (\gamma_0, \gamma_1, \mu_1,
\beta_{x1}, \beta_{x0}, \sigma_1, \sigma_0, \phi )$ is not identifiable
by finding $\bmath \xi^{'} \neq \bmath \xi$, but $\pr (y | R; \bmath \xi) = \pr
(y | R; \bmath \xi^{'})$. If the last equation holds, we have
...
\end{align*}
and keep all the other parameters in $\bmath \xi^{'}$ the same. We can
still have the same distribution of $y|R; \bmath \xi$ but with different
$\bmath \xi$. Therefore, $\bmath \xi$ is not identifiable One solution is to
...
$\beta_{10} = \beta_{10}^{'}$, therefore, $\bmath \xi$ is identifiable.

For identifiability issue with the heterogeneous model described in
section \ref{ch3:sec:settings}, it is easy to show there is no trouble
...
\subsection{Proof of Lemma \ref{ch3:sec:lemma}}
\label{ch3:sec:proof}
\begin{itemize}
\item Denote
  \begin{displaymath}
...
\item For integrating over a normal distribution with mean $\mu$ and
  ...
  \end{align*}
  The last equation holds by (\ref{ch3:eq:int})
...
\subsection{Goodness of Fit Check for Simulated Data}
\label{ch3:sec:gofsim}
\begin{itemize}
\item Scenario 1
\begin{enumerate}
\item Normal:
