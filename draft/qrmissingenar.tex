...
\begin{abstract} In this paper, we consider the problem of
longitudinal quantile regression using pattern mixture models when
there is monotone missingness. Our proposed pattern mixture models
specify the marginal quantile regression parameters, as well as the
conditional distribution, thus allowing sensitivity analysis, a key
component in inference for incomplete data. For computation of the
likelihood of the parametric model, we proposed a novel way to
simplify the integral. Both frequentist and Bayesian methods are
illustrated. The model is applied on a clinical trial study for weight
management.
\end{abstract}
