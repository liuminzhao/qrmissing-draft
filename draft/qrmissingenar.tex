...
Michael G. Perri$^{3,***}$\email{mperri@phhp.ufl.edu} \\
  ...
  $^2$Section of Integrative Biology, Division of Statistics and Scientific
  Computation\\
...
\begin{abstract}
  ...
  parameters. Our approach allows elicitation and sensitivity analysis
 which are an essential component in inference for incomplete
  data. To facilitate computation of the likelihood, we propose a
  novel way to obtain analytic forms for required integrals.
...
From (\ref{eq:model}), we have $E(Y_{i2}|Y_{i1}, R = 0) = \Delta_{i2} + \bmath x_{i2}^T \bmath h_2^{(1)} + Y_{i1}\beta_{y,1}^{(1)}$ and
by sensitivity parameters settings, $\beta_{y, 1}^{(1)} = \beta_{y, 1}^{(\ge 2)} + \eta_1^{(1)}$.
Thus we can incorporate the sensitivity parameters in
the distribution of $Y_2|Y_1, R=0$ via the following restriction:
\begin{displaymath}
  \Delta_{i2} + \bmath x_{i2}^T \bmath h_2^{(1)} + E(Y_{i1}|R=0)(\beta_{y,1}^{(\ge 2)} +\eta_1^{(1)} - 1) = 3.6 \mbox{kg}.
\end{displaymath}
...
Web Appendix referenced in Section~\ref{sec:real} showing goodness of fit check for Tours data is available with this paper at the Biometrics website on Wiley Online Library.
The supplemental materials associated to this paper also include R package ``qrmissing'' which can be downloaded from author's website.
\vspace*{-8pt}
