...
% Time-stamp: <liuminzhao 02/04/2014 17:58:22>
...
$\chi(\bmath x_{ij}, \bmath y_{ij^{-}})$ is a function of $(\bmath x_{ij}, \bmath y_{ij^{-}})$, which is the mean of the unobserved data distribution and allows sensitivity analysis by varying assumptions on $\chi$.
For example, here we specify
\begin{equation}
\label{eq:model2}
\chi(\bmath x_{ij}, \bmath y_{ij^{-}}) = \Delta_{ij}  + \bmath y_{ij^{-}}^T \bmath \beta_{y,j-1}^{(\ge j)}+ h_{0}^{(k)} ,
\end{equation}
$h_0^{(k)}$ is a sensitivity parameter which indicates the intercept shift.
Comparing (\ref{eq:model}) and (\ref{eq:model2}), the unknown distribution of
$\pr_k(y_{ij}|\bmath y_{ij^{-}})$ when $k < j$ is characterized as an intercept shift from observed distribution.
...
The parameters $h_0^{(k)}$ are sensitivity parameters (not identified by the observed data).
