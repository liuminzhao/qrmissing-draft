...
% Time-stamp: <liuminzhao 02/14/2014 16:08:57>
...
\begin{abstract}
  ...
  To facilitate computation of the likelihood, we propose a novel way to obtain analytic forms for the required integrals.  We conduct simulations to examine the robustness of our approach to modelling assumptions and compare its performance to competing approaches.
...
In terms of Bayesian inference, both parametric and semiparametric Bayesian appraoches have been proposed in the literature \citep{yu2001,walker1999,hanson2002,reich2010}.
...
$\Delta_{ij}$ are subject/time specific intercepts determined by the parameters in (\ref{eq:model}), (\ref{eq:model2}) and (\ref{eq:marg}); more details are given in what follows.
...
\begin{equation}
\label{eq:model2}
\chi(\bmath x_{i}, \bmath y_{ij^{-}}) = \Delta_{ij}  + \bmath y_{ij^{-}}^T \bmath \beta_{y,j-1}^{(\ge j)}+ h_{0}^{(k)} ,
\end{equation}
$h_0^{(k)}$ is a sensitivity parameter and $\Delta_{ij}$ depends on $x_i$.
...
\item \textbf{$\Delta_{ij}, 2\leq j \leq J: $}
  ...
  \begin{align*}
    ...
    & = \int\dots\int
    \Phi \left( \frac{\bmath x_{i}^T \bmath \gamma_j - \Delta_{ij} - h_0^{(1)} - \bmath y_{ij^{-}}^T \bmath \beta_{y,j-1}^{(1)}}{\sigma_{j}^{(1)}} \right) \\
    & \quad   dF(Y_{i(j-1)}|S=1, \bmath x_{i}, \bmath Y_{i(j-1)^{-}}) \cdots d F (Y_{i1} | S = 1, \bmath x_{i}), \\
...
We did not include the similar results in this article,
because there are not large differences for estimates for $Y_2$ under MNAR vs MAR.
...
BZ for middle quantiles regression inference even when
...
The R package ``qrmissing'' can be downloaded from the author's website.
