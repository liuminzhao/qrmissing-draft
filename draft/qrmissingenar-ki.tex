...
% Time-stamp: <liuminzhao 02/17/2014 16:33:53>
...
\end{align*}
when $\xi(\bmath x_i, \bmath y_{ij^{-}})$ is the linear form in (\ref{eq:model2}) and $\beta_{y_{j-1}, j-1} > 0$.
%\Delta_{ij} - h_0^{(1)} - \bmath y_{ij^{-}}^T \bmath %\beta_{y,j-1}^{(1)}}{\sigma_{j}^{(1)}}
...
Our approach cab ne implemented with the R package ``qrmissing'' which can be downloaded from the author's website: \url{https://github.com/liuminzhao/qrmissing}.
