...
sink('sim-lp-mnar-mnar-0516.txt')
rm(list = ls())
library(xtable)
library(qrmissing)
library(doMC)
...
write.table(result, file = "sim-lp-mnar-mnar-0516-result.txt", row.names = F, col.names = F)
...
result <- read.table('sim-lp-mnar-mnar-0516-result.txt')
