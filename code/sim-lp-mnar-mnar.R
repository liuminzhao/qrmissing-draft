#!/bin/Rscript
##' Time-stamp: <liuminzhao 08/28/2013 00:25:53>
...
##' 2013/08/26 SP = (2, 0, 0, 0), using qrmissing package

sink('sim-lp-mnar-mnar-0828.txt')
rm(list = ls())
library(xtable)
library(qrmissing)
library(doMC)
...
write.table(result, file = "sim-lp-mnar-mnar-0828-result.txt", row.names = F, col.names = F)
sendEmail(subject="simulation-lp-mnar", text="done", address="liuminzhao@gmail.com")
...
quan2 <- function(y, x, tau, quan){
  return(quan - .5*pLD(y - 0.5  + (1.5*x), tau = tau/(1 + alpha*x)) - .5*pLD(y - (2.5 - x),tau = tau/(1 + alpha*x)))
}
...
result <- read.table('sim-lp-mnar-mnar-0828-result.txt')
