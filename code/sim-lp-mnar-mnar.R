#!/bin/Rscript
##' Time-stamp: <liuminzhao 07/15/2013 00:31:32>
...
sink('sim-bottai-lp-mnar-0715.txt')
rm(list = ls())
source('BiMLESigma.R')
source('Bottai.R')
source('sendEmail.R')
...
write.table(result, file = "sim-lp-mnar-0715-result.txt", row.names = F, col.names = F)
...
result <- read.table('sim-lp-mnar-0715-result.txt')
