#!/bin/Rscript
##' Time-stamp: <liuminzhao 07/17/2013 15:41:47>
...
sink('sim-lp-mnar-mnar-0717.txt')
rm(list = ls())
source('BiMLESigma.R')
source('Bottai.R')
source('sendEmail.R')
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  for (i in 1:n){
    if (R[i] == 1){
      ...
      y[i, 2] <- 2 - x[i] - y[i, 1]/2 + rnorm(1, sd = sqrt(6*w[i]/tau))*(1 + alpha*x[i])
    }
  }
...
write.table(result, file = "sim-lp-mnar-mnar-0717-result.txt", row.names = F, col.names = F)
...
quan2 <- function(y, x, tau, quan){
  return(quan - .5*pLD(y + 1 + (2*x), tau = tau/(1 + alpha*x)) - .5*pLD(y - (4 - 0*x),tau = tau/(1 + alpha*x)))
}
...
result <- read.table('sim-lp-mnar-mnar-0717-result.txt')
