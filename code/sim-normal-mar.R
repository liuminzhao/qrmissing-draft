#!/bin/Rscript
##' Time-stamp: <liuminzhao 08/28/2013 00:15:00>
...
write.table(result, file = "sim-normal-mar-result-0828.txt", row.names = F, col.names = F)
...
quan2 <- function(y, x, tau){
  return(tau - .5*pnorm(y, 0.5-1.5*x, (1+alpha*x)*sqrt(5/4)) - .5*pnorm(y, 0.5-1*x, (1+alpha*x)*sqrt(5/4)))
}
...
result <- read.table('sim-normal-mar-result-0828.txt')
