#!/bin/Rscript
##' Time-stamp: <liuminzhao 05/16/2014 11:48:46>
...
sink('sim-m1-mnar-mnar-0516.txt')
rm(list = ls())
library(qrmissing)
library(xtable)
library(doMC)
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  mod1 <- QRMissingBiMixMLE(y, R, X, tau = 0.1, K = 2, model = 'slope', sp = c(2, 0))
  mod3 <- QRMissingBiMixMLE(y, R, X, tau = 0.3, K = 2, model = 'slope', sp = c(2, 0))
  mod5 <- QRMissingBiMixMLE(y, R, X, tau = 0.5, K = 2, model = 'slope', sp = c(2, 0))
  mod7 <- QRMissingBiMixMLE(y, R, X, tau = 0.7, K = 2, model = 'slope', sp = c(2, 0))
  mod9 <- QRMissingBiMixMLE(y, R, X, tau = 0.9, K = 2, model = 'slope', sp = c(2, 0))
...
write.table(result, file = "sim-m1-mnar-mnar-result-0516.txt", row.names = F, col.names = F)
sendEmail(subject="simulation-m1-mnar-MNAR", text="done", address="liuminzhao@gmail.com")
...
result <- read.table('sim-m1-mnar-mnar-result-0516.txt')
