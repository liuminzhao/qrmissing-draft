#!/bin/Rscript
##' Time-stamp: <liuminzhao 01/04/2015 22:46:54>
...
sink('sim-normal-mnar-mar-0101.txt')
rm(list = ls())
library(xtable)
library(qrmissing)
library(doMC)
...
write.table(result, file = "sim-normal-mnar-mar-0101-result.txt", row.names = F, col.names = F)
sendEmail(subject="simulation-normal-MNAR-mar", text="done", address="liuminzhao@gmail.com")
...
## result <- read.table('sim-normal-mnar-mar-0626-result.txt')
