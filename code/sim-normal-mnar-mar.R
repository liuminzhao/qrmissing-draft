#!/bin/Rscript
##' Time-stamp: <liuminzhao 08/07/2013 22:40:32>
...
sink('sim-normal-mnar-mar-0807.txt')
rm(list = ls())
library(compiler)
library(quantreg)
library(rootSolve)
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  for (i in 1:n){
    if (R[i] == 1){
      ...
      y[i, 1] <- -2 - x[i] +(1 + alpha*x[i])*rnorm(1)
      y[i, 2] <- 2 - x[i] - 0.5 * y[i, 1] + (1+alpha*x[i])*rnorm(1)
    }
  }
...
write.table(result, file = "sim-normal-mnar-mar-0807-result.txt", row.names = F, col.names = F)
...
quan2 <- function(y, x, tau){
  return(tau - .5*pnorm(y, -1.5*x, (1+alpha*x)*sqrt(5/4)) - .5*pnorm(y, 3-.5*x, (1+alpha*x)*sqrt(5/4)))
}
...
result <- read.table('sim-normal-mnar-mar-0807-result.txt')
