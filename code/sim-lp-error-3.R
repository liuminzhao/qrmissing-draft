...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  for (i in 1:n){
    if (R[i] == 1){
      y[i, 1] <- 4 + x[i] + (1 + alpha*x[i])*rnorm(1, 0, sd = sqrt(8*w[i]/tau))
      y[i, 2] <- 1 - x[i] - y[i, 1]*1/2 + rnorm(1,sd = sqrt(6*w[i]/tau))*(1 + alpha*x[i])
    } else {
      y[i, 1] <- -4 - x[i] + (1 + alpha*x[i])* rnorm(1, sd = sqrt(8*w[i]/tau))
...
write.table(result, file = "sim-lp-error-0713.txt", row.names = F, col.names = F)
...
quan1 <- function(y, x, tau, quan){
  return(quan - .5*pLD(y - 4 - x, tau = tau/(1 + alpha*x) ) - .5*pLD(y + 4 + x, tau = tau/(1 + alpha*x)))
}
...
quan2 <- function(y, x, tau, quan){
  return(quan - .5*pLD(y + 1 + (1.5*x), tau = tau/(1 + alpha*x)) - .5*pLD(y - (3 - 0.5*x),tau = tau/(1 + alpha*x)))
}
...
result <- read.table('sim-lp-error-0713.txt')
