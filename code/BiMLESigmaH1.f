c===========================================================
c$$$  
C$$$  Time-stamp: <liuminzhao 04/13/2013 22:52:37>
...
CCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE TargetEqn1H1f(delta,gamma,beta,sigma,heter,tau,p,x,f)
...
C     SOLVE DELTA 1
CCCCCCCCCCCCCCCCCCCC
      SUBROUTINE SolveDelta1H1f(param, tau, x, root)
...
C     INITIAL 
      ...
      call TargetEqn1H1f(m,gamma,beta,sigma,heter,tau, p, x, fm)
      call TargetEqn1H1f(b,gamma,beta,sigma,heter,tau, p, x, fb)
      call TargetEqn1H1f(a,gamma,beta,sigma,heter,tau, p, x, fa)
...
CCCCCCCCCCCCCCCCCCCC
      ...
      subroutine targeteqn2h1f(delta2, gamma2, beta1, beta2, sigma1,
...
CCCCCCCCCCCCCCCCCCCC
      ...
      subroutine solvedelta2h1f(param, tau, x, delta1, root)
      implicit none
      real*8 param(18)
...
C     INITIAL 
      ...
      call targeteqn2h1f(m, gamma2, beta1, beta2, sigma1,
     &     heter1, sigma2, lambda, tau, p, x, delta1, fm)
      call targeteqn2h1f(b, gamma2, beta1, beta2, sigma1,
     &     heter1, sigma2, lambda, tau, p, x, delta1, fb)
      call targeteqn2h1f(a, gamma2, beta1, beta2, sigma1,
...
CCCCCCCCCCCCCCCCCCCC
      ...
      subroutine negloglikelihoodh1(y, R, x, delta1, delta2,param,n,nll)
      implicit none
      integer n
      integer R(n)
...
CCCCCCCCCCCCCCCCCCCC
      ...
      subroutine PartialH1f(param, tau, x, y, R, n, pp)
      implicit none
      integer n
      real*8 param(18)
      integer R(n)
      ...
      do i = 1, 18
         if (i .ne. 9 .and. i.ne.10.and.i.ne.11.and.i.ne.13) then
            ...
            do j = 1,n 
               call SolveDelta1H1f(param1, tau, x(j), delta1p(j))
               call SolveDelta1H1f(param2, tau, x(j), delta1m(j))
             call solvedelta2h1f(param1,tau,x(j),delta1p(j),delta2p(j))
             call solvedelta2h1f(param2,tau,x(j),delta1m(j),delta2m(j))
            end do

         call negloglikelihoodh1(y, R, x, delta1p, delta2p,param1,n,ll1)
         call negloglikelihoodh1(y, R, x, delta1m, delta2m,param2,n,ll2)
...
CCCCCCCCCCCCCCCCCCCC
      ...
      subroutine BiQRGradientH1f(y,R,x,tau,n,niter,param,paramsave)
      implicit none
      integer n, niter
      integer R(n)
      ...
      do while (dif > 0.00001 .and. iter .le. niter)
         call PartialH1f(param, tau, x, y, R, n, pp)
         do i = 1, 18
...
C            param(i) = max(param(i), -1/maxx)
         ...
         do j = 1, n
            call SolveDelta1H1f(param, tau, x(j), delta1(j))
            call solvedelta2h1f(param,tau,x(j),delta1(j),delta2(j))
         end do
         call negloglikelihoodh1(y, R, x, delta1, delta2,param,n,nll)
         dif = abs(nll - nll0)
         nll0 = nll
         do i = 1, 18
