#!/bin/Rscript
##' Time-stamp: <liuminzhao 07/19/2013 00:02:16>
...
##' LP distribution
##' 2013/07/17 use +-4 +- 2x
##' 2013/07/19 with correct SP = (2, 0,0,0,0)

sink('sim-lp-mnar-mnar-0719.txt')
rm(list = ls())
source('BiMLESigma.R')
source('Bottai.R')
source('sendEmail.R')
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  for (i in 1:n){
    if (R[i] == 1){
      ...
      y[i, 2] <- 3 - x[i] - y[i, 1]/2 + rnorm(1, sd = sqrt(6*w[i]/tau))*(1 + alpha*x[i])
    }
  }
  ...
  mod1 <- BiQRGradient(y, R, X, tau = 0.1, sp = c(2, 0, 0, 0, 0), method = 'heter2')
  mod3 <- BiQRGradient(y, R, X, tau = 0.3, sp = c(2, 0, 0, 0, 0), method = 'heter2')
  mod5 <- BiQRGradient(y, R, X, tau = 0.5, sp = c(2, 0, 0, 0, 0), method = 'heter2')
  mod7 <- BiQRGradient(y, R, X, tau = 0.7, sp = c(2, 0, 0, 0, 0), method = 'heter2')
  mod9 <- BiQRGradient(y, R, X, tau = 0.9, sp = c(2, 0, 0, 0, 0), method = 'heter2')
...
write.table(result, file = "sim-lp-mnar-mnar-0719-result.txt", row.names = F, col.names = F)
sendEmail(subject="simulation-lp-mnar-mnar", text="done", address="liuminzhao@gmail.com")
...
quan2 <- function(y, x, tau, quan){
  return(quan - .5*pLD(y + 1 + (2*x), tau = tau/(1 + alpha*x)) - .5*pLD(y - (5 - 0*x),tau = tau/(1 + alpha*x)))
}
...
result <- read.table('sim-lp-mnar-mnar-0719-result.txt')
