#!/bin/Rscript
##' Time-stamp: <liuminzhao 05/16/2014 11:33:49>
...
write.table(result, file = "sim-mle-m1-mar-result-0516.txt", row.names = F, col.names = F)
sendEmail(subject="simulation-mle-m1-MAR", text="done", address="liuminzhao@gmail.com")
...
quan2 <- function(y, x, tau){
  return(tau - 1/8*pnorm(y, -3 - 1.5*x, sqrt(5/4)) - 1/8*pnorm(y, 1 - 1.5*x, sqrt(5/4)) - 1/8*pnorm(y, -1 - 1.5*x, sqrt(5/4)) - 1/8*pnorm(y, 3 - 1.5*x, sqrt(5/4)) - 1/8*pnorm(y, -1 - .5*x, sqrt(5/4)) - 1/8*pnorm(y, 3 - .5*x, sqrt(5/4)) - 1/8*pnorm(y, 1 - .5*x, sqrt(5/4))- 1/8*pnorm(y, 5 - .5*x, sqrt(5/4)))
}
...
result <- read.table('sim-mle-m1-mar-result-0516.txt')
