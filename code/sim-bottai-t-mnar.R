#!/bin/Rscript
##' Time-stamp: <liuminzhao 07/14/2013 08:08:38>
...
sink('sim-bottai-t-mnar-0714.txt')
rm(list = ls())
source('BiMLESigma.R')
source('Bottai.R')
source('sendEmail.R')
...
write.table(result, file = "sim-t-mnar-0714-result.txt", row.names = F, col.names = F)
...
result <- read.table('sim-t-mnar-0714-result.txt')
