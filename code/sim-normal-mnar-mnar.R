...
sink('sim-normal-mnar-mnar-0506.txt')
rm(list = ls())
library(qrmissing)
library(xtable)
library(doMC)
...
write.table(result, file = "sim-normal-mnar-mnar-result-0506.txt", row.names = F, col.names = F)
...
result <- read.table('sim-normal-mnar-mnar-result-0506.txt')
