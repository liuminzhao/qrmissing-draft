#!/bin/Rscript
##' Time-stamp: <liuminzhao 04/22/2014 22:57:13>
...
sink('sim-normal-mnar-mnar-0422.txt')
rm(list = ls())
library(qrmissing)
library(xtable)
library(doMC)
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  mod1 <- QRMissingBiMixMLE(y,R,X,tau=0.1, K = 2, sp = 2)
  mod3 <- QRMissingBiMixMLE(y,R,X,tau=0.3, K = 2, sp = 2)
  mod5 <- QRMissingBiMixMLE(y,R,X,tau=0.5, K = 2, sp = 2)
  mod7 <- QRMissingBiMixMLE(y,R,X,tau=0.7, K = 2, sp = 2)
  mod9 <- QRMissingBiMixMLE(y,R,X,tau=0.9, K = 2, sp = 2)
...
write.table(result, file = "sim-normal-mnar-mnar-result-0422.txt", row.names = F, col.names = F)
...
result <- read.table('sim-normal-mnar-mnar-result-0422.txt')
