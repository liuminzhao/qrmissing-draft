#!/bin/Rscript
##' Time-stamp: <liuminzhao 05/16/2014 11:38:53>
...
sink('sim-mle-m1-mnar-mnar-0516.txt')
rm(list = ls())
library(qrmissing)
library(xtable)
library(doMC)
...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  mod1 <- QRMissingBi(y, R, X, tau = 0.1, model = 'slope',sp=c(2,0))
  mod3 <- QRMissingBi(y, R, X, tau = 0.3, model = 'slope',sp=c(2,0))
  mod5 <- QRMissingBi(y, R, X, tau = 0.5, model = 'slope',sp=c(2,0))
  mod7 <- QRMissingBi(y, R, X, tau = 0.7, model = 'slope',sp=c(2,0))
  mod9 <- QRMissingBi(y, R, X, tau = 0.9, model = 'slope',sp=c(2,0))
...
write.table(result, file = "sim-mle-m1-mnar-mnar-result-0516.txt", row.names = F, col.names = F)
sendEmail(subject="simulation-mle-m1-mnar-MNAR", text="done", address="liuminzhao@gmail.com")
...
result <- read.table('sim-mle-m1-mnar-mnar-result-0516.txt')
