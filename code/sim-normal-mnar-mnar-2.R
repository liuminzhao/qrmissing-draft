...
##' 2013/07/18 use +-4 +-2x, and MNAR with SP = (2, 0, 0, 0, 0)

sink('sim-normal-0718-mnar.txt')
rm(list = ls())
source('BiMLESigma.R')
source('sendEmail.R')
source('Bottai.R')
...
write.table(result, file = "sim-normal-mnar-0718-result.txt", row.names = F, col.names = F)
...
quan1 <- function(y, x, tau){
  return(tau - .5*pnorm(y, 4+x*2, 1 + 0.5*x) - .5*pnorm(y, -4-x*2, 1 + 0.5*x))
}
...
result <- read.table('sim-normal-mnar-0718-result.txt')
