...
result <- foreach(icount(boot), .combine = rbind) %dopar% {
  ...
  for (i in 1:n){
    if (R[i] == 1){
      y[i, 1] <-  12 + x[i] + (1 + alpha*x[i])*rnorm(1, 0, sd = sqrt(winv[i]))
      y[i, 2] <-  y[i, 1]*1/2  +  (1 + alpha*x[i])*rnorm(1, 0, sd = sqrt(3*winv[i]/4))
    } else {
      y[i, 1] <-  -12 - x[i] +(1 + alpha*x[i])*rnorm(1, 0, sd = sqrt(winv[i]))
      y[i, 2] <-  y[i, 1]*1/2  +  (1 + alpha*x[i])*rnorm(1, 0, sd = sqrt(3*winv[i]/4))
    }
...
quan1 <- function(y, x, tau){
  return(tau - .5*pt((y - 12- x)/(1+alpha*x), df = 3) - .5*pt((y+12+x)/(1+alpha*x),df = 3))
}
...
quan2 <- function(y, x, tau){
  return(tau - .5*pt((y - 6- 0.5*x)/(1+alpha*x), df = 3) - .5*pt((y + 6 + 0.5*x)/(1 + alpha*x),df = 3))
}
